    ...
    transformIndexHtml(html) {
      ...
      try {
        ...
        if (config.analytics?.posthog?.token) {
          // JSON.stringify so config-controlled values are safely escaped
          // when inlined into the script (quotes, </script>, etc.)
          const token = JSON.stringify(String(config.analytics.posthog.token)).replace(/</g, '\\u003c');
          const apiHost = JSON.stringify(String(config.analytics.posthog.apiHost || 'https://us.i.posthog.com')).replace(/</g, '\\u003c');
